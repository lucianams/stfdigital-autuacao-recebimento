...
function config($stateProvider: ng.ui.IStateProvider, properties: app.support.constants.Properties) {
    ...
    $stateProvider.state("app.tarefas.recebimento-devolucao-assinatura", {
        ...
        resolve : {
            protocolos: /** @ngInject */ ($stateParams: ng.ui.IStateParamsService, $q: ng.IQService) =>
                    $q.when([$stateParams["informationId"]]),
                    devolucoes: ["app.recebimento.devolucao-assinatura.DevolucaoAssinaturaService", "protocolos",
                        (devolucaoAssinaturaService: DevolucaoAssinaturaService, protocolos: number[]) => {
